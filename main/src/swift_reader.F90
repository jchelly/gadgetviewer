...
!
  type(result_type) function swift_open(fname, isnap, rinfo)
    ...
    do ifile = 0, n-1, 1
       ...
       if(hdferr.ne.0)then
          swift_open%string="Unable to read NumPart_Total from file"
          hdferr = hdf5_close_file()
          return
       endif
...
!
  type (result_type) function swift_read(isnap, rinfo)
    ...
    if(ios.eq.0)then
       if(.not.fexist)then
          swift_read%success = .false.
          swift_read%string  = "File does not exist: "//trim(fname)
          return
       endif
    else
       swift_read%success = .false.
       swift_read%string  = "Unable to access file" 
       return
    endif
    ...
    if(ios.ne.0)then
       swift_read%success = .false.
       swift_read%string  = "Unable to open Eagle snapshot file"
       return
    endif
    ...
    do ispecies = 1, 6, 1
       ...
       if(np.gt.0)then
          ...
          if(ios.ne.0)then
             swift_read%success = .false.
             swift_read%string  = "Unable to read Velocity dataset"
             call close_snapshot(snap)
             call particle_store_empty(pdata)
          ...
          if(ios.ne.0)then
             swift_read%success = .false.
             swift_read%string  = "Unable to read Mass dataset"
             call close_snapshot(snap)
             call particle_store_empty(pdata)
